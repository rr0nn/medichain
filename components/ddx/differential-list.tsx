...
/**
 * @fileoverview Renders the ranked differential list and diagnosis-specific supporting evidence details.
...
export function DifferentialList({
  ...
  const getPathDetails = (
    ...
    return {
      ...
      evidenceTypeLabel:
        path.evidenceType === "feature" ? featureMatch?.featureType : undefined,
    };
  };
          ...
          <details key={d.diagnosisKey} className="group space-y-2">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-3 rounded-lg border border-[color:var(--glass-border)] bg-background/45 px-3 py-2.5 text-left shadow-[inset_0_1px_0_var(--glass-highlight)] [&::-webkit-details-marker]:hidden">
              ...
              <span className="shrink-0 rounded-full bg-background px-2.5 py-1 text-[11px] text-muted-foreground">
                Evidence support score {d.score.toFixed(2)}
              </span>
            </summary>
                ...
                <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-3 py-2.5 text-left [&::-webkit-details-marker]:hidden">
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Shows the matched presentation, source details, and
                        supporting evidence for each path.
                      </p>
                    </div>
                  </div>
                  <span className="shrink-0 rounded-full bg-background px-2.5 py-1 text-[11px] text-muted-foreground">
                          ...
                          <div className="space-y-3 rounded-lg border border-[color:var(--glass-border)] bg-background/45 p-3">
                            ...
                            <div className="space-y-1.5">
                              ...
                              {pathDetails.clinicalPresentationSources.length > 0 ? (
                                <div className="flex flex-wrap gap-1.5">
                                  {pathDetails.clinicalPresentationSources.map(
                                    (source) => (
                                      <span
